...
class ClientSystemController(RestController):
    ...
    def get_one(self, client_id):
        ...
        if client:
            client_dict = {'client_system_id': client.client_system_id,
    ...
    @expose()
    def delete(self, client_system_id):
        aux=model.del_client_system(client_system_id=client_system_id)
        if aux:
            response.status = 200
        else:
            response.status = 404
    ...
    @expose('json')
    def edit_detail(self, client_id, detail):
        client = model.get_client_system(client_id)
        if client.client_system_id:
            if model.edit_client_system(client_id, client_id, detail):
                response.status = 200
            else:
                response.status = 403
        else:
